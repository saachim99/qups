function [img] = shot_gather_mig_t_plot_2(x, z, c_x_z, f, rxdata_f, txdata_f, aawin, kwargs)
arguments
    x
    z
    c_x_z
...
%initalise pressure map
hprop = figure;
hax= gca;
hi_prop = him = imagesc(hax, 1000*x, 1000*z, db(abs(img)/max(abs(img(:)))), [-80, 0]);
...
switch kwargs.version
    ...
    case 2
        ...
        for z_idx = 1:numel(z)-1
            ...
            p_tx_curr_ifft = ifft(p_tx_curr,[],2);
            imagesc(abs(p_tx_curr_ifft))
            N{z_idx} = getframe();
        ...
        % Save Accumulation of Image in Frequency Domain
       % movie2gif(M, 'FreqDomain2.gif');
        movie2gif(N, 'prop_test.gif');
        %movie(M,1,'limitrate');
        %saveas(M);
